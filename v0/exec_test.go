...
var tVal = &T{
	...
	Tmpl:              Must(new(Set).Parse(`{{define "x"}}test template{{end}}`)), // "x" is the value of .X
}
...
func testExecute(execTests []execTest, template *Set, t *testing.T) {
	...
	for _, test := range execTests {
		...
		b.Reset()
		err = tmpl.Execute(b, test.name, test.data)
		switch {
...
func TestDelims(t *testing.T) {
	...
	for i := 0; i < len(delimPairs); i += 2 {
		...
		text := fmt.Sprintf(`%sdefine "x"%s`, trueLeft, trueRight)
		text += fmt.Sprintf(`%s.Str%s`, trueLeft, trueRight)
		// Now add a comment
		text += trueLeft + "/*comment*/" + trueRight
		// Now add  an action containing a string.
		text += trueLeft + `"` + trueLeft + `"` + trueRight
		text += fmt.Sprintf(`%send%s`, trueLeft, trueRight)
		// At this point text looks like `{{.Str}}{{/*comment*/}}{{"{{"}}`.
		tmpl, err := new(Set).Delims(left, right).Parse(text)
		if err != nil {
			t.Fatalf("delim %q text %q parse err %s", left, text, err)
		}
		var b = new(bytes.Buffer)
		err = tmpl.Execute(b, "x", value)
		if err != nil {
...
func TestExecuteError(t *testing.T) {
	b := new(bytes.Buffer)
	tmpl, err := new(Set).Parse(`{{define "x"}}{{.MyError true}}{{end}}`)
	if err != nil {
		t.Fatalf("parse error: %s", err)
	}
	err = tmpl.Execute(b, "x", tVal)
	if err == nil {
...
func TestExecError(t *testing.T) {
	tmpl, err := new(Set).Parse(execErrorText)
	if err != nil {
		t.Fatal("parse error:", err)
	}
	var b bytes.Buffer
	err = tmpl.Execute(&b, "one", 5) // 5 is out of range indexing "hi"
	if err == nil {
		t.Fatal("expected error")
	}
	const want = `template: three:4:20: executing "three" at <index "hi" $>: error calling index: index out of range: 5`
	got := err.Error()
	if got != want {
		t.Errorf("expected\n%q\ngot\n%q", want, got)
	}
