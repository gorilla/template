...
var multiParseTests = []multiParseTest{
	...
	{"one", `{{define "foo"}} FOO {{end}}`, noError,
		[]string{"foo"},
		[]string{`{{define "foo"}} FOO {{end}}`}},
	{"two", `{{define "foo"}} FOO {{end}}{{define "bar"}} BAR {{end}}`, noError,
		[]string{"foo", "bar"},
		[]string{`{{define "foo"}} FOO {{end}}`, `{{define "bar"}} BAR {{end}}`}},
	// errors
...
func TestMultiParse(t *testing.T) {
	for _, test := range multiParseTests {
		template, err := new(Set).Parse(test.input)
		switch {
...
var templateFileExecTests = []execTest{
	{"test", `{{define "test"}}{{template "template1"}}{{template "template2"}}{{end}}`, "\ntemplate1\ny\n\ntemplate2\nx\n", 0, true},
}
...
func TestRedefinition(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "duplicated template name") {
