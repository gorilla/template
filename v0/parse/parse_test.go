...
func testParse(doCopy bool, t *testing.T) {
	for _, test := range parseTests {
		tmpl, err := Parse(test.name, fmt.Sprintf(`{{define "%s"}}%s{{end}}`, test.name, test.input), "", "", builtins)
		switch {
		...
		var result string
		if doCopy {
			result = tmpl[test.name].Copy().String()
		} else {
...
func TestErrors(t *testing.T) {
	for _, test := range errorTests {
		_, err := Parse(test.name, fmt.Sprintf(`{{define "%s"}}%s{{end}}`, test.name, test.input), "", "")
		if err == nil {
